...
# Global Phone API JAVA REST Sample Code

This is a sample code for Melissa's Global Phone API.
It is written in JAVA and accesses the API through REST.
This sample is intended to show the functionality of the Global Phone Web Service and how to call the service in JAVA.
...
Inside the folder will be the build under `globalphone` as well as an all-in-one `globalphone.zip`.
...
To run the example, navigate to `./target/globalphone/bin/` and run `globalphone.bat`.
